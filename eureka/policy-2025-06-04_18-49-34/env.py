...
def compute_reward(obs_buf: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    """
    Compute reward using trained neural network
    
    Args:
        obs_buf: Full observation buffer (should be 211-dimensional)
    ...
    checkpoint_path = "/home/gx22/Desktop/isaacgym/python/Eureka/eureka/checkpoints/checkpoint_epoch_200.pt"
    model = MLPReward(obs_dim=211)
    
    if checkpoint_path.endswith('model_weights.pt'):
        model.load_state_dict(torch.load(checkpoint_path, map_location=obs_buf.device))
    else:
        checkpoint = torch.load(checkpoint_path, map_location=obs_buf.device)
        model.load_state_dict(checkpoint['model_state_dict'])
    
    model.eval()
    model.to(obs_buf.device)
    
    # Compute rewards
    batch_size = obs_buf.shape[0]
    rewards = torch.zeros(batch_size, device=obs_buf.device)
    
    with torch.no_grad():
        for i in range(batch_size):
            reward = model(obs_buf[i])
            rewards[i] = reward.squeeze()
    
    reward_components = {
        "neural_network_reward": rewards,
    }
