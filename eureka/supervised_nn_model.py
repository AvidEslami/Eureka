...
validation_batch = pairwise_data[:validation_batch_size]
pairwise_data = pairwise_data[validation_batch_size:]
best_validation_loss = float('inf')
best_epoch = -1
print(f"Epoch 0, Validation Loss: {loss_func(validation_batch, nn_model).item():.4f}")
for epoch in range(num_epochs):
    total_loss = 0.0
    for i in range(0, len(pairwise_data), batch_size):
        batch = pairwise_data[i:i+batch_size]
        optimizer.zero_grad()
        loss = loss_func(batch, nn_model)
        loss.backward()
        optimizer.step()
        total_loss += loss.item()
    avg_loss = total_loss / (len(pairwise_data) / batch_size)
    validation_loss = loss_func(validation_batch, nn_model).item()
    print(f"Epoch {epoch+1}/{num_epochs}, Loss: {avg_loss:.5f}, Validation Loss: {validation_loss:.5f}")
    if validation_loss < best_validation_loss:
        best_validation_loss = validation_loss
        # best_epoch = epoch
        # Save the best model
        print("Saving New Best: {:.5f}".format(best_validation_loss))
        torch.save(nn_model.state_dict(), "best_nn_reward_model.pth")
        torch.save(nn_model, "best_nn_reward_model_full.pth")

## Now plot some of the validation results to see if the gist is captured
# Use first 1000 timesteps from validation batch
import matplotlib.pyplot as plt
nn_model = torch.load("best_nn_reward_model_full.pth")
pairwise_data = validation_batch[:1000]
