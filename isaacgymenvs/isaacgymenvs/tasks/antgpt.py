...
@torch.jit.script
def compute_reward(potentials: torch.Tensor, prev_potentials: torch.Tensor, root_states: torch.Tensor, actions: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Scalar weights and parameters (trainable)
    forward_reward_weight = 5.0         # weight for forward velocity reward component
    forward_reward_temp = 1.0           # temperature parameter for forward velocity reward component

    ctrl_penalty_weight = 0.1           # weight for control penalty (action magnitude)
    ctrl_penalty_temp = 0.5             # temperature parameter for control penalty smoothing

    contact_penalty_weight = 0.5        # weight for contact force penalty to encourage smooth contacts
    contact_penalty_temp = 5.0          # temperature parameter for contact penalty scaling

    survival_reward_weight = 1.0        # constant reward for staying alive

    # Extract forward velocity from potentials difference (already negative distance / dt)
    # Potential is negative distance/dt, so difference potentials - prev_potentials approximates forward velocity
    forward_velocity = potentials - prev_potentials  # This is roughly velocity along forward direction

    # Reward for forward progress: encourage positive forward velocity
    forward_reward = forward_reward_weight * torch.clamp(forward_velocity, min=0.0)

    # Control cost: penalize large actions to encourage smooth control
    control_penalty = ctrl_penalty_weight * torch.sum(actions * actions, dim=-1)
    
    # Contact force penalty for smooth contact (if sensor forces are accessible, but not here)
    # Since contact forces are unavailable as input, omit this term to comply with input spec

    # Survival reward (constant positive reward to keep ant alive)
    survival_reward = survival_reward_weight * torch.ones_like(forward_reward)

    # Total reward
    reward = forward_reward + survival_reward - control_penalty

    reward_components = {
        "forward_reward": forward_reward,
        "control_penalty": -control_penalty,  # negative penalty for logging
