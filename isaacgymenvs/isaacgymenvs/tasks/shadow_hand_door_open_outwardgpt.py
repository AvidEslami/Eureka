...
class ShadowHandDoorOpenOutwardGPT(VecTask):
    ...
    def compute_reward(self, actions):
        # self.rew_buf[:], self.rew_dict = compute_reward(self.left_hand_pos, self.door_left_handle_pos, self.door_left_handle_rot)
...
# @torch.jit.script
# def compute_reward(door_left_handle_pos: torch.Tensor, door_right_handle_pos: torch.Tensor, left_hand_pos: torch.Tensor, right_hand_pos: torch.Tensor, goal_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
#     # Scalar weights and parameters 
#     handle_dist_weight = 1.0
#     hand_dist_weight = 0.7
#     goal_dist_weight = 0.9
    
#     handle_dist_temp = 0.5 
#     hand_dist_temp = 0.5 
#     goal_dist_temp = 0.7
    
#     handle_dist_thresh = 0.1
#     hand_dist_thresh = 0.1 
...
#     door_handle_pos = (door_left_handle_pos + door_right_handle_pos) / 2.0 # Calculate the position of the door handle. This is necessary if it is possible to push both sides of the door
    
#     # calculate distance between the door handle and the positions of the left and right hand
#     left_handle_dist = torch.norm(door_handle_pos - left_hand_pos, dim=-1)
#     right_handle_dist = torch.norm(door_handle_pos - right_hand_pos, dim=-1)
#     handle_dist = torch.minimum(left_handle_dist, right_handle_dist) # The handle distance is the minimum of the distances calculated
    
#     handle_reward = torch.exp(-handle_dist_temp * handle_dist) # calculate the reward based on distance
#     handle_success = (handle_dist < handle_dist_thresh).float() # check if the distance is less than the threshold
    
#     left_hand_dist = torch.norm(goal_pos - left_hand_pos, dim=-1)
#     right_hand_dist = torch.norm(goal_pos - right_hand_pos, dim=-1)
#     hand_dist = torch.minimum(left_hand_dist, right_hand_dist)
    
#     hand_reward = torch.exp(-hand_dist_temp * hand_dist)
#     hand_success = (hand_dist < hand_dist_thresh).float()
    
#     goal_dist = torch.norm(goal_pos - door_handle_pos, dim=-1)
    
#     goal_reward = torch.exp(-goal_dist_temp * goal_dist)
#     goal_success = (goal_dist < goal_dist_thresh).float()
    
#     # calculate total reward
#     total_reward = handle_dist_weight * handle_reward + hand_dist_weight * hand_reward + goal_dist_weight * goal_reward

#     return total_reward, {"total_reward": total_reward, "handle_dist_reward": handle_reward, "hand_dist_reward": hand_reward, "goal_dist_reward": goal_reward, "handle_success": handle_success, "hand_success": hand_success, "goal_success": goal_success}
...
def compute_reward(
    ...
    total_reward = mlp_reward_model(obs_buf).squeeze(-1)
    rew_dict = {"mlp_rew": total_reward}
